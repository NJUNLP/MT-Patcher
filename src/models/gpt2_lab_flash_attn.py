...
class GPTPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, model_name, config, *args, strict=True, device=None, dtype=None,
        ...
        state_dict = remap_state_dict_lab_gpt2(state_dict, config)
...
class GPTModel(GPTPreTrainedModel):
    ...
    def __init__(self, config: GPT2Config, process_group=None, device=None, dtype=None):
        ...
        use_rms_norm = getattr(config,"use_rms_norm",False)
        self.parallel_block = getattr(config,"parallel_block",False)

        embed_dim = config.n_embd
        if process_group is None:
            self.embeddings = GPT2Embeddings(
                embed_dim, vocab_size, config.max_position_embeddings,
                word_embed_proj_dim=None, **factory_kwargs
            )
        ...
        # This option is for Lab GPT2
        if embed_dim != self.config.hidden_size:  # factorization
            self.pre_token_proj = nn.Linear(embed_dim, self.config.hidden_size)
            self.post_token_proj = nn.Linear(self.config.hidden_size, embed_dim)
        else:
            self.pre_token_proj = None
            self.post_token_proj = None
        print(f"my hacking here, embed_dim: {embed_dim}\thidden_size: {self.config.hidden_size}")
        ...
        if self.prenorm:
            ...
            self.ln_f = norm_cls(embed_dim, eps=config.layer_norm_epsilon,
                                 **factory_kwargs)
        if process_group is not None:
        ...
        if self.prenorm:
            if not self.fused_dropout_add_ln:
                ...
                if self.post_token_proj is not None:  # 映射下
                    residual = self.post_token_proj(residual)
...
def remap_state_dict_lab_gpt2(state_dict, config):
    ...
    def key_mapping_mlp(key):
        key = re.sub(r'^transformer.h.(\d+).mlp.c_fc.bias', r'transformer.layers.\1.mlp.fc1.bias', key)
    ...
    # Attention
    for d in range(config.num_hidden_layers):
        state_dict.pop(f'transformer.h.{d}.attn.bias')  # We don't store this bias
        state_dict.pop(f'transformer.h.{d}.attn.masked_bias')  # We don't store this bias
    ...
    def key_mapping_attn(key):
        ...
        key = re.sub(r'^transformer.h.(\d+).attn.c_proj.bias', r'transformer.layers.\1.mixer.out_proj.bias', key)
        return key
...
if __name__ == "__main__":
    ...
    model = GPTLMHeadModel.from_pretrained(args.model_dir,config,dtype=torch.half)
